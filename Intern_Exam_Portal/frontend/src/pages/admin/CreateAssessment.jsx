...
import {
    ...
    FileSpreadsheet, ChevronDown, ChevronUp, Layers
} from 'lucide-react';
...
export default function CreateAssessment() {
    ...
    const [expandedId, setExpandedId] = useState(null);  // which assessment is expanded for upload

    // ── Per-assessment MCQ set upload state ──────────────────────────────
    // uploadState[assessmentId] = { setName, file, uploading, result }
    const [uploadState, setUploadState] = useState({});

    const fileRefs = useRef({});  // one file input ref per assessment
    ...
    const handleCreate = async (e) => {
        ...
        try {
            ...
            setTitle(''); setJobPosition(''); setExpLevel('fresher');
            loadAssessments();
        } catch (err) {
    ...
    const setUpload = (id, patch) => setUploadState(prev => ({
        ...prev, [id]: { ...getUpload(id), ...patch },
    }));
                            ...
                            {showTitleDrop && filteredTitles.length > 0 && (
                                <ul className="ca-suggestions">
                                    {filteredTitles.map(s => (
                                        <li key={s} onMouseDown={() => { setTitle(s); setShowTitleDrop(false); }}>
                                            {s}
                                        </li>
                                    ))}
                                </ul>
                        ...
                        <div className="ca-field ca-exp-field">
                            ...
                            <div className="ca-exp-group">
                                <label className={`ca-exp-option ${expLevel === 'fresher' ? 'active' : ''}`}>
                                    ...
                                    <div className="ca-exp-label">
                                        <span className="ca-exp-name">Fresher</span>
                                    </div>
                                </label>
                                <label className={`ca-exp-option ${expLevel === 'experienced' ? 'active' : ''}`}>
                                    ...
                                    <div className="ca-exp-label">
                                        <span className="ca-exp-name">Experienced</span>
                                    </div>
                                </label>
                            </div>
                        </div>
